function maxEle(arr) {
    var maxi = -Infinity; //taking maximum element
    var idx = -1; // taking the index of the maximum element
...
function barGraph(arr) {
    var maxi = maxEle(arr); // giving the maxi a value of maximum element of array
    var n = arr.length; // deciding how long the graph is going to be printed
    for (var floor = maxi; floor >= 1; floor--) {
        // checking if the value of array is max by giving the name of floor
        var s_1 = ' '; //assigning the s string to put value later on
        for (var i = 0; i <= n; i++) {
        ...
        console.log(s_1); //printing the got output of each line.
    }
